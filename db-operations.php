...
{ 
  include ("db-connect.php");
  $query = "SELECT * FROM `b2b_users` WHERE isdeleted = 'N' AND isblocked = 'N' AND type = 'user'";
  $result = mysqli_query($con,$query);
  $values = array();
  $i=0;
...
function addItem($name,$quantity,$categoty,$description,$image)
{
  $today = date("y.m.d");
  include ("db-connect.php");
  $query = "INSERT INTO `b2b_item` (`id`, `name`, `image`, `description`, `quantity`, `category`, `created_on`, `isdeleted`, `isblocked`) VALUES (NULL, '".$name."', '".$image."', '".$description."', '".$quantity."', '".$categoty."', '".$today."', 'N', 'N');";
...
function editItemDetails($id,$name,$quantity,$category,$description,$image)
{
  $today = date("y.m.d");
  include ("db-connect.php");
  $query = "UPDATE `b2b_item` SET `name` = '".$name."', `image` = '".$image."', `description` = '".$description."', `quantity` = '".$quantity."', `category` = '".$category."', `created_on` = '".$today."' WHERE `b2b_item`.`id` = '".$id."'";
